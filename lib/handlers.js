...
var Phrase = require('../db/models/Phrase');
    ...
    User.findOne({ phone: phone }, function(err,userData){
      if(!err && userData){
        ...
        if(hashedPassword == userData.hashedPassword){
          ...
          var token = new Token(tokenObject);
...
handlers._tokens.get = function(data,callback){
  var id = typeof(data.queryStringObject.token_id) == 'string' && data.queryStringObject.token_id.trim().length == 20 ? data.queryStringObject.token_id.trim() : false;
  if(id){
...
handlers._tokens.delete = function(data,callback){
  var id = typeof(data.payload.id) == 'string' && data.payload.id.trim().length == 24 ? data.payload.id.trim() : false;
  if(id){
...
handlers._phrases.post = function(data,callback){
  var phrase = data.payload.phrase.length > 0 ? data.payload.phrase : false;
  var translation =  data.payload.translation.length > 0 ? data.payload.translation : false;
  var owner = data.payload.userId.length > 0 ? data.payload.userId : false;

  if(phrase && translation ){
    var phraseObj  = { phrase, translation, owner };
    var phrase = new Phrase(phraseObj);
    phrase.save().then(() => {
      callback(200);
    })
    .catch(err => {
    ...
    //     callback(200, false);
    //   } else {
    //     callback(400, {'Error' : 'Cannot save the data'})
    //   }
    // })
...
handlers._phrases.get = function(data,callback){
  var userId = typeof(data.queryStringObject.userId) == 'string' ? data.queryStringObject.userId : false;
  if(userId){
...
handlers._phrases.delete = function(data,callback){
  // var userId = typeof(data.payload.userId) == 'string' ? data.payload.userId : false;
  var phraseId = typeof(data.payload.phraseId) == 'string' ? data.payload.phraseId : false;
